...
class Kb:
    ...
    def encode(self, texts):
        """嵌入编码"""
        return np.array([embeddings(model='bge-base-zh', prompt=text)['embedding'] for text in texts])
    ...
    def rerank(self, docs, query):
        ...
        keyword_scores = [self.bm25_index.get_score(doc_idx, tokenized_query) for doc_idx in range(len(docs))]
...
if __name__ == "__main__":
    ...
    for q in test_questions:
        ...
        # 解析响应格式
        if "###回答###" in response:
            think, answer = response.split("###回答###", 1)
            print(f"Assistant思考过程:\n{think.strip()}")
            print(f"\n最终答案:\n{answer.strip()}")
        else:
